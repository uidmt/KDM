...
showPanel(0,'#f44336');    /* Popup Form Development Start */
	window.addEventListener("load", function(){
		setTimeout(function open(event){
			document.querySelector(".content-containe").style.display = "block";
		},
		15000
		)
	});
	...
	document.querySelector("#close").addEventListener("click", function(){
		document.querySelector(".content-containe").style.display = "none";
	})
    /* Popup Form Development End *//* Blog Page Custom JavaScript - Extracted from blog/default.aspx */

// Hero Search Filter & Topic Chips
function kdmFilterBlogCards() {
	var input = document.getElementById('kdmBlogHeroSearch');
	if (!input) return;
...
function kdmSetTopicFilter(topic, el) {
...
function showPage(pageNumber, shouldScroll = true) {
	...
	// 1. Show/Hide cards based on index
	...
	updatePaginationButtons(pageNumber, allCards.length);
...
function updatePaginationButtons(activePage, totalCardsCount) {
	...
	for (let i = 1; i <= totalPages; i++) {
		...
		btn.innerText = i;
		btn.className = (i === activePage) ? "tab-button active" : "tab-button";
